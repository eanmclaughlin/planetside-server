...
var PopulationTracker = function (PlanetsideDatabase) {
    ...
    this.processEvent = function (event) {
        if (event.event_name == 'PlayerLogout') {
            ...
            if (event.event_name == 'PlayerLogin')
                onlinePlayer.login = event.timestamp;
            ...
            PlanetsideDatabase.collections.Generated.OnlinePlayer.findOneAndUpdate({character_id: event.character_id}, onlinePlayer, {upsert: true}, (err) => {
                if (err) console.log(err);
            })
        }
    };

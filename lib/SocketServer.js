...
var SocketServer = function (express, PlanetsideDatabase, AlertTracker, PopulationTracker, opts) {
    ...
    setInterval(() => {
        PopulationTracker.getFullPopulation().then((pop) => {
            this.live.emit('population', pop);
        });
    }, 5000);
    ...
    this.live.on('connection', (client) => {
        ...
        client.on('resetPop', () => {
            PopulationTracker.clearCache();
            PlanetsideDatabase.collections.Generated.OnlinePlayer.remove({}, (err) => {
    ...
    PlanetsideSocket.on('event', (event) => {
        if (PlanetsideDatabase.collections.Events[event.event_name]) {
            ...
            CensusCache.fillEvent(event).then((filledEvent) => {
                PopulationTracker.processEvent(filledEvent);
